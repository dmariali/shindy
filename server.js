...
chatNsp.on("connection", (socket) => {
  socket.on("join_room", (user) => {
    ...
    console.log(
      "User List on Userconnected: ",
      user_list.map((user) => user.name)
    );
  });
  ...
  socket.on("new_chat_message", (data, roomId) => {
    ...
    chatNsp
      .to(roomId)
      .emit("new_chat_message", { message: data.message, name: data.user });
  });

  socket.on("disconnecting", () => {
    socket_rooms = Object.keys(socket.rooms).splice(
      1,
